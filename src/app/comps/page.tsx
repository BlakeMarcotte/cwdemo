...
function ActiveCompsTable({
  ...
  // Track original indices through filtering/sorting
  const indexedData = useMemo(() => data.map((r, i) => ({ ...r, _idx: i })), [data]);
  ...
  }, [indexedData, search, filterSubmarket, filterType, filterFootprint, filterCounty, filterCity, sortKey, sortDir]);
...
function OldCompsTable({
  ...
  const filtered = useMemo(() => {
    ...
    if (filterFootprint) rows = rows.filter((r) => r.footprint === filterFootprint);
    return sortRows(rows, sortKey, sortDir);
  }, [indexedData, search, filterSubmarket, filterType, filterFootprint, sortKey, sortDir]);
...
export default function CompsPage() {
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      {/* Tab content */}
      {tab === "active" ? (
        <ActiveCompsTable data={activeData} onUpdate={handleActiveUpdate} />
      ) : (
        <OldCompsTable data={oldData} onUpdate={handleOldUpdate} />
      )}
    </div>
  );
}
